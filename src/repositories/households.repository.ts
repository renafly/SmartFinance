...
export class HouseholdsRepository extends BaseRepository<"households"> {
  ...
  async createHousehold(name: string): Promise<RepoResult<Household>> {
    ...
    const household = (Array.isArray(data) ? data[0] : data) as
      Household | undefined;

    if (!household) {
      return {
        data: null,
        error: new Error("Household creation did not return a household."),
      };
    }
  ...
  async acceptInvitationByToken(
    token: string,
  ): Promise<RepoResult<{ household_id: string; role: HouseholdRole }>> {
    const { data, error } = await (
      this.client as unknown as {
        rpc: (
          fn: string,
    ...
    if (!row) {
      return {
        data: null,
        error: new Error("Invitation response did not return a household."),
      };
    }
    ...
    if (!householdId || !role) {
      return {
        data: null,
        error: new Error("Invitation response payload is incomplete."),
      };
    }
  ...
  async transferOwnership(
    ...
    if (!row || typeof row.success !== "boolean" || !row.message) {
      return {
        data: null,
        error: new Error("Invalid RPC response for transfer ownership."),
      };
    }

    return {
      data: { success: row.success, message: row.message },
      error: null,
    };
  }

  /** Remove a household member. */
  async removeMemberByRpc(
    householdId: string,
    ...
    if (!row || typeof row.success !== "boolean" || !row.message) {
      return {
        data: null,
        error: new Error("Invalid RPC response for remove member."),
      };
    }
  ...
  async leaveHousehold(
    ...
    if (!row || typeof row.success !== "boolean" || !row.message) {
      return {
        data: null,
        error: new Error("Invalid RPC response for leave household."),
      };
    }
