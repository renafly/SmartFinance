  ...
  return (
    <Page title={t('transfers.title')} subtitle={t('transfers.subtitle')}>
      ...
      <Section title={t('transfers.scheduledTitle')} subtitle={t('transfers.scheduledSubtitle', { count: (recurringQuery.data ?? []).length })}>
        ...
        {visibleRules.length ? (
          <Table columns={[
            { label: t('recurring.titleLabel'), flex: 2 }, { label: t('transfers.route'), flex: 2.2 },
            ...
            {visibleRules.map((item: any) => {
              ...
              const sourceName = item.account?.name ?? accounts.find((account: any) => account.id === item.account_id)?.name ?? t('transfers.sourceAccount');
              const route = kind === 'recurring-transfer'
                ? t('transfers.transferRoute', { from: sourceName, to: destinationName ?? t('transfers.destination') })
                : sourceName;
              return (
                <TableRow key={item.id}>
                  <TableCell flex={2}><View style={styles.ruleTitle}><Text style={{ color: colors.text, fontWeight: typography.fontWeight.bold as any }}>{item.title}</Text><View style={styles.ruleMeta}><Badge label={item.is_active ? t('active') : t('inactive')} tone={item.is_active ? 'success' : 'neutral'} /><Text style={[styles.ruleKind, { color: colors.textSecondary }]}>{t(kind === 'recurring-transfer' ? 'transfers.types.recurringTransfer' : 'transfers.types.recurringTransaction')}</Text></View></View></TableCell>
                  <TableCell flex={2.2}><View style={styles.routeCell}><Text style={[styles.routeAccount, { color: colors.text }]}>{sourceName}</Text>{kind === 'recurring-transfer' ? <Ionicons name="arrow-forward-outline" size={16} color={colors.primary} /> : null}<Text style={[styles.routeAccount, { color: colors.text }]}>{kind === 'recurring-transfer' ? destinationName ?? t('transfers.destination') : route}</Text></View></TableCell>
                  ...
                  <TableCell align="right"><Text style={[styles.ruleAmount, { color: colors.primary }]}>{formatCurrency(item.amount)}</Text></TableCell>
...
const styles: any = StyleSheet.create({
  formFields: { gap: spacing(3) }, fieldGroup: { gap: spacing(2) }, fieldLabel: { fontWeight: typography.fontWeight.semibold as any }, pillRow: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing(2) }, filters: { gap: spacing(2), marginBottom: spacing(4) }, ruleTitle: { gap: spacing(1.5), alignItems: 'flex-start' }, ruleMeta: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: spacing(1.5) }, ruleKind: { fontSize: typography.fontSize[12] }, routeCell: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: spacing(1) }, routeAccount: { fontWeight: typography.fontWeight.semibold as any }, ruleAmount: { fontSize: typography.fontSize[16], fontWeight: typography.fontWeight.extraBold as any }, menuButton: { width: spacing(9), height: spacing(9), alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderRadius: radius.md }, dateButton: { minHeight: spacing(11), borderWidth: 1, borderRadius: radius.mdPlus, paddingHorizontal: spacing(3.5), flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, datePicker: { gap: spacing(2), padding: spacing(3), borderWidth: 1, borderRadius: radius.lg }, inlineActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: spacing(2) }, modalBackdrop: { flex: 1, justifyContent: 'center', padding: spacing(4) }, modalScroll: { flexGrow: 1, justifyContent: 'center' }, modalCard: { alignSelf: 'center', gap: spacing(3.5), borderWidth: 1, borderRadius: radius.xl, padding: spacing(4.5), maxWidth: '100%' }, historyCard: { alignSelf: 'center', gap: spacing(3.5), borderWidth: 1, borderRadius: radius.xl, padding: spacing(4.5), maxWidth: '100%' }, historyRow: { gap: spacing(1.5), borderTopWidth: 1, paddingTop: spacing(2) }, menuCard: { alignSelf: 'center', gap: spacing(2.5), borderWidth: 1, borderRadius: radius.xl, padding: spacing(4.5), maxWidth: '100%' }, modalTitle: { fontSize: typography.fontSize[20], fontWeight: typography.fontWeight.extraBold as any }, modalActions: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'flex-end', gap: spacing(2) }, menuAction: { flexDirection: 'row', alignItems: 'center', gap: spacing(2), paddingHorizontal: spacing(3), paddingVertical: spacing(3), borderRadius: radius.lg, borderWidth: 1 },
});
