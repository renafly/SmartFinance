-- The Transactions list's "Balance after" column showed a plain "--" for
-- every transfer row: list_transaction_movements only ever returned ONE
-- balance-after value (`balance_after_transaction`, the source/outgoing
-- account's balance immediately after the transfer left it), with no
-- equivalent for the destination account, so the UI had nothing to show
-- for the "money arriving" side.
--
-- Adds `destination_balance_after_transaction` to list_transaction_movements
-- -- the destination account's balance immediately after the transfer
-- arrived, via the same split-aware `balance_after_transaction()` already
-- used for the source leg (`outgoing`), just applied to `incoming` instead.
-- Null for every non-transfer row (regular transactions only ever touch one
-- account, already covered by `balance_after_transaction`).
--
-- Postgres refuses `create or replace function` when the change adds a
-- column to a `returns table (...)` row shape ("cannot change return type
-- of existing function ... Row type defined by OUT parameters is
-- different"), so the old signature has to be dropped first.
