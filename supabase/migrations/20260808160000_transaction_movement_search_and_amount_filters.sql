-- Extends list_transaction_movements with the filters needed to fix the
-- "movements" view (income+expense, transfers hidden) and to support the new
-- free-text search and amount-range filters on the Transactions screen:
--
-- 1. p_exclude_transfers: previously the client asked for p_kind = null (all
--    kinds) and stripped transfer rows out of the result *after* fetching a
--    full page. Since a page can be entirely transfers, this could silently
--    return fewer than p_limit visible rows and confuse infinite-scroll
--    "load more" triggering. Doing the exclusion in the WHERE clause keeps
--    every returned page correctly sized.
-- 2. p_search: case-insensitive match against title, notes, or merchant_name
--    (added to the CTEs/return type below -- transfers surface the outgoing
--    leg's merchant_name, matching how they already surface its category).
-- 3. p_min_amount / p_max_amount: inclusive amount-range bounds.
...
'Lists paginated transaction and completed-transfer movements, including the account balance after the transaction or transfer source transaction. Supports sorting by date, amount, or title (p_sort: newest, oldest, amount_asc, amount_desc, title_asc, title_desc). Filtering by a parent category (p_category_id) also includes its subcategories; filtering by a subcategory matches only that subcategory. p_exclude_transfers drops transfer rows server-side (used by the "movements" view so pages are always fully populated). p_search matches title/notes/merchant_name case-insensitively. p_min_amount/p_max_amount bound the amount column inclusively.';
...
-- The old 13-arg overload is superseded by the 17-arg one above; drop it so
-- PostgREST doesn't have two overloads with ambiguous default-argument calls.
drop function if exists public.list_transaction_movements(uuid, text, uuid, uuid, uuid, uuid, boolean, uuid, date, date, text, integer, integer);
