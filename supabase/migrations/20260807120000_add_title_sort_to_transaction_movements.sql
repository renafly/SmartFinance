-- Adds "title_asc"/"title_desc" as valid p_sort values so the Transactions
-- list can sort by name server-side (required for pagination to stay
-- correct: without this the app can only re-sort whichever page has
-- already been fetched, not the full result set).
...
'Lists paginated transaction and completed-transfer movements, including the account balance after the transaction or transfer source transaction. Supports sorting by date, amount, or title (p_sort: newest, oldest, amount_asc, amount_desc, title_asc, title_desc).';
